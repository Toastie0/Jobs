...
public class PostgreSQLConnectionHandler implements IConnectionHandler {
   ...
   public void retrieveCredentials(ConfigurationSection var1) {
      if (var1.contains("host")) {
         ...
         this.port = var1.getInt("port", 5432);
      } else {
   ...
   public BaseConnectionSource connect() {
      String var1 = "jdbc:postgresql://" + this.host + ":" + this.port + "/" + this.database + "?autoReconnect=true";
      return new JdbcPooledConnectionSource(var1, this.username, this.password, new PostgresDatabaseType());
   }

   @Override
   public BaseConnectionSource connectHikari() {
      HikariConfig var1 = new HikariConfig();
      var1.setJdbcUrl("jdbc:postgresql://" + this.host + ":" + this.port + "/" + this.database);
