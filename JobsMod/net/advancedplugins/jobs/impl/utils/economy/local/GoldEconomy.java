...
public class GoldEconomy implements AdvancedEconomy {
   ...
   public boolean chargeUser(Player var1, double var2) {
      if (!AManager.hasAmount(var1, Material.GOLD_INGOT, (int)var2)) {
         return false;
      } else {
         AManager.removeItems(var1.getInventory(), Material.GOLD_INGOT, (int)var2);
         return true;
      }
   }
   ...
   public double getBalance(Player var1) {
      return AManager.getAmount(var1, Material.GOLD_INGOT);
   }

   @Override
   public boolean giveUser(Player var1, double var2) {
      ASManager.giveItem(var1, Collections.nCopies((int)var2, Material.GOLD_INGOT).stream().map(ItemStack::new).toArray(ItemStack[]::new));
      return true;
   }
}
